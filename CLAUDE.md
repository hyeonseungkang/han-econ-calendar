...
- `src/han_econ_calendar/fetch.py` — downloads raw economic-calendar exports:
  ...
  - Saves each response to `<workspace_dir>/temp/<year>-<month>_<natcd>.xls` (`temp/` is gitignored).
- `src/han_econ_calendar/generate_ics.py` — converts fetched exports into calendars:
  - Reads every `.xls` file in `temp/`, parsing `<year>-<month>_<natcd>` out of the filename (`natcd` matches `fetch.py`'s output) and grouping files by `natcd`.
